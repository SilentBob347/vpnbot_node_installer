...
def ai_dns_records(path: Path | None = None) -> list[tuple[str, str]]:
    """Load exact host/IP records generated by vpnbot-ai-access.

    The AI service remains the source of truth.  This adapter only projects an
    explicitly activated fragment into the existing kernel and Hysteria allow
    layers so a Russian SNI-proxy IP is not rejected by the broader RU policy.
    """
    ...
    for number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        records.append((normalized_host, address))
    if not records:
...
def exception_addresses(config: dict[str, Any]) -> dict[int, list[str]]:
    ...
    for _host, raw in ai_dns_records():
        address = ipaddress.ip_address(raw)
        addresses[address.version].add(str(address))
...
def render_hysteria_acl(config: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(f"direct(suffix:{host})" for host, _ip in ai_dns_records())
